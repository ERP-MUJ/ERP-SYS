...
export async function GET(
  ...
  try {
    ...
    if (isNaN(assignedKpiId)) {
      return NextResponse.json({ error: 'Invalid assigned_kpi_id format' }, { status: 400 });
    }
    ...
    if (!kpiData) {
      return NextResponse.json({ error: 'Assigned KPI not found' }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch assigned KPI data' }, { status: 500 });
  }
}
