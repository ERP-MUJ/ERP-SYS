...
export default function TableFormRenderer({ name, elements, id, description, className = "" }: TableFormRendererProps) {
  ...
  const [complexValue, setComplexValue] = useState<string | boolean | number | null>(null)
  ...
  const removeRow = (index: number) => {
    setEntries(entries.length === 1 ? [{}] : entries.filter((_, i) => i !== index))
  }

  const updateEntry = (rowIndex: number, elementId: string, value: string | number | boolean | null) => {
  ...
  return (
    <Card className={className}>
      ...
      <CardContent>
              ...
              <TableRow>
                {tableElements.map((element) => (
                  <TableHead key={element.id} className="whitespace-nowrap">
                    {element.attributes.label}{element.attributes.required && " *"}
                  </TableHead>
                ))}
            ...
            <TableBody>
              {entries.map((entry, rowIndex) => (
                <TableRow key={rowIndex}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {complexElements.map((element) => {
                          ...
                          return (
                            <Button key={element.id} variant={hasValue ? "default" : "outline"} size="sm" onClick={() => openComplexEditor(rowIndex, element)} className="text-xs h-7">
                              {element.attributes.label}{hasValue && " ✓"}
                            </Button>
                          )
                        })}
                      </div>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="icon" onClick={() => removeRow(rowIndex)} className="h-8 w-8 text-red-500 hover:text-red-600 hover:bg-red-50">
                      <Trash2 size={16} />
                    </Button>
                  </TableCell>
                </TableRow>
        ...
        {entries.length === 0 && <div className="text-center py-4 text-gray-500">No entries yet. Add your first entry.</div>}
      </CardContent>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{activeElement?.attributes.label}</DialogTitle></DialogHeader>
          <div className="py-4">
            {activeElement?.type === "textarea" && (
              <div className="space-y-2">
                <Label htmlFor={activeElement.id}>{activeElement.attributes.label}{activeElement.attributes.required && " *"}</Label>
                <Textarea id={activeElement.id} placeholder={activeElement.attributes.placeholder} rows={activeElement.attributes.rows} value={typeof complexValue === "string" || typeof complexValue === "number"
  ? String(complexValue)
  : ""}
 onChange={(e) => setComplexValue(e.target.value)} />
            ...
            {activeElement?.type === "radio" && (
              <div className="space-y-2">
                <Label>{activeElement.attributes.label}{activeElement.attributes.required && " *"}</Label>
                <RadioGroup value={typeof complexValue === "string" || typeof complexValue === "number"
  ? String(complexValue)
  : ""}
 onValueChange={setComplexValue}>
                  {activeElement.attributes.options?.map((option: { label: string; value: string }, index: number) => (
                    <div key={index} className="flex items-center space-x-2">
                      <RadioGroupItem value={option.value} id={`${activeElement.id}-${index}`} />
                      <Label htmlFor={`${activeElement.id}-${index}`}>{option.label}</Label>
                    </div>
                  ))}
                </RadioGroup>
              </div>
            ...
            {activeElement?.type === "file" && (
              <div className="space-y-2">
                ...
                <div className="border-2 border-dashed rounded-md p-6 text-center">
                  ...
                  <p className="text-sm text-gray-500">{complexValue ? `File selected: ${complexValue}` : "No file selected"}</p>
                  <Button type="button" variant="outline" size="sm" className="mt-2" onClick={() => setComplexValue(`file-${Date.now()}.pdf`)}>Select File</Button>
                </div>
                <p className="text-xs text-gray-500">{activeElement.attributes.multiple ? "Multiple files allowed" : "Single file only"} • Accepted formats: {activeElement.attributes.acceptedFileTypes || "All files"}</p>
              </div>
            )}
          </div>
...
function renderTableCellInput(
  ...
  rowIndex: number,
  updateEntry: (rowIndex: number, elementId: string, value: string | number | boolean | null) => void,
) {
    ...
    case "number":
      return (
        <Input
          ...
          onChange={(e) =>
            updateEntry(rowIndex, id, e.target.value ? Number(e.target.value) : "")
          }
          placeholder={attributes.placeholder}
          min={attributes.min}
          max={attributes.max}
      ...
      return (
        <div className="flex items-center justify-center">
          <Checkbox
            ...
            onCheckedChange={(checked) => updateEntry(rowIndex, id, checked === true)}
          />
        </div>
      );
    default:
